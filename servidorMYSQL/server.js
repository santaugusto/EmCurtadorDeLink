...
// Defina a variável urlMapping fora de qualquer função
const urlMapping = {};
...
app.post('/shorten', express.json(), (req, res) => {
  ...
  // Criar a URL curta usando a função shortenURL
  ...
  conexao.query(
    'INSERT INTO urlsLinkCort (urloriginal, urlcortada) VALUES (?, ?)',
    [longURL, shortURL],
    (err, results) => {
      if (err) {
        console.error('Erro ao inserir dados no banco de dados:', err);

        // Verifique se o erro é devido a uma violação de chave única
        if (err.code === 'ER_DUP_ENTRY') {
          return res.status(400).json({ error: 'A URL já foi encurtada anteriormente.' });
        }
      ...
      // Envie a URL curta como resposta
